...
<div class="background">
  <div class="wrapperUser">
    ...
    <button class="prev" { currentQuestionNumber > 0 && (
            (click)="onPrev()">{{'Quiz.Prev Question' | translate}}</button>
    <button class="next" { quizQuestions.length > currentQuestionNumber+1 && (
            (click)="onNext($event)">{{'Quiz.Next Question' | translate}}</button>
    <button class="next" { currentQuestionNumber == null || quizQuestions.length == currentQuestionNumber+1 && (
