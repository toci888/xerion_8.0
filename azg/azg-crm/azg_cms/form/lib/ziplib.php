...
//bzdura nie dostime tylko convert tam wszystko jest
require ('convert.php');

/*
* ok, ten ziplib wygl¹da w miare obiecuj¹co, ale trzeba go troche zmienic, komentarze stopniowo beda provided, najpierw sprawdzenie czy to dziala w ogole
* 
* biblioteka zostala lekko zmodyfikowana, roznia sie tylko niektore naglowki - informacje o wersji itd, usunalem tez komentarz do archiwum uznajac go za zbedny - zawsze mozna go przywrocic
*/
...
public function Ziplib ($arch_name)
{
...
public function zl_add_file($srcfilename, $filename, $comp = "") 
{
	global $settings;
    ...
    else 
    {
		...
		if ($comp[0] == "b" OR $comp[0] == "g" OR $comp[0] == "n") 
        $comp_type = $comp[0];
		if (preg_match("/[0-9]/i",$comp[1])) 
        $comp_lvl = $comp[1];
	}

	// ok, let's get this bitch tidy:
	// first adding a file
    //odczyt calego pliku danych
    $uchwyt = fopen($srcfilename, 'r');
    $data = fread($uchwyt, filesize($srcfilename));
...
public function zl_pack() 
{
    ...
    ///zapis archiwum w pliku na dysku
    $handle = fopen($this->arch_name, 'w');
    fwrite($handle, $this->archive.$cdss.$cdse);
    fclose($handle);
	//return $this->archive.$cdss.$cdse;
}

//najpewniej odczyt archiwum
