<?php
    ...
    class UtilsUI
    {
        ...
        protected static $naglowekUmowy = '<table border="0" width="100%"><tr><td><img src="../../zdj/logo_l.jpg"  width=181 border=0></img>        
                    ...
                    <td align="right" style="font-size: 12px;"><b><u>Siedziba Główna:</u><br />ul. Szarych Szeregów 34d,<br>45-285 Opole<br />tel./fax.(077)402-75-20</b></td>
        ...
        public static function WyswietlTab1Poz ($wynik, $WyswKolumny, $naglowki, $index, $HiddenNazwa, $akcje, $nagIndAk = null, $przycIndAk = null, $popup = null, $links = null, $zagn = null, $dod_hid_id = null) //zalozenie ze jest mozliwe jedno zagniezdzenie, jak nad tyym zapanujer moze byc tablica
        {
            ...
            foreach ($wynik as $wiersz)
            {
                ...
                if (isset($akcje[Akcja::$kasowanie]))
                {
                    ...
                    {
                        echo '<td bgcolor="'.$kolor.'">';
                        KontrolkiHtml::DodajSubmit('kasowanie', $wiersz[$index], $tlumaczenia->Tlumacz ($_SESSION[$jezyk], tags::$kasuj).'.', 'onclick="'.$HiddenNazwa.'.value = this.id;'.$oncl_dod_hid.'"');
                        echo '</td>';
                    }
                }
                ///dodane 6.03
                if (is_array($przycIndAk))
                {
                    ...
                    {
                        echo '<td bgcolor="'.$kolor.'">';
                        KontrolkiHtml::DodajSubmit($klucz, $wiersz[$index], $tlumaczenia->Tlumacz ($_SESSION[$jezyk], $wartosc).'.', 'onclick="'.$HiddenNazwa.'.value = this.id;'.$oncl_dod_hid.'"');
                        echo '</td>';
                    }
                }
                if ($popup != null)
        ...
        public static function GenerujUmowaKlient($klient, $transakcja, $prowizja) //ustalic liste koniecznych parametrow metody
        {
            ...
            if ($transakcja == tags::$kupno)
            {
                echo 'Wynagrodzenie POŚREDNIKA za określoną wyżej usługę wynosi '.$prowizja.' % netto + vat (22%) wartości przedmiotu kupna (nabycia) wg ceny zakupu, jednak nie mniej niż 1000 zł. (dotyczy nieruchomości o cenie ofertowej poniżej 34.000 zł.) płatne przez ZAMAWIAJĄCEGO w dniu podpisania protokołu uzgodnień lub umowy przedwstępnej - najpóźniej jednak w chwili podpisania Aktu Notarialnego. W przypadku zawarcia przez ZAMAWIAJĄCEGO umowy najmu nieruchomości wynagrodzenie POŚREDNIKA ustala się w wysokości jednomiesięcznego czynszu najmu netto + vat (22%).';
            }
            else
            {
                echo 'Wynagrodzenie POŚREDNIKA za określoną wyżej usługę najmu wynosi '.$prowizja.' % jednomiesięcznego czynszu najmu  netto + vat (22%) płatne przez ZAMAWIAJĄCEGO w dniu podpisania protokołu uzgodnień lub umowy najmu.';
            }
            echo '<li>';
            if ($transakcja == tags::$kupno)
            {
                echo 'W przypadku zawarcia przez ZAMAWIAJĄCEGO lub Jego bliskich lub osób pozostających w zależności służbowej umowy kupna (nabycia) nieruchomości podanej na wskazaniu adresowym z pominięciem POŚREDNIKA, ZAMAWIAJĄCY zapłaci POŚREDNIKOWI karę umowną w wysokości 4 % wartości netto + vat (22%) przedmiotu kupna (nabycia) wg ceny ofertowej podanej na wskazaniu adresowym, a w przypadku nieruchomości o wartości ofertowej do 34.000 zł. nie mniej niż 2.000 zł. netto + vat (22%), a w przypadku najmu dwukrotny czynsz netto + vat (22%) - wg ceny podanej na Liście Wskazań Adresowych. Obowiązek zapłacenia tej kary wraz z odsetkami ustawowymi za każdy dzień zwłoki, powstaje z dniem zawarcia umowy kupna (nabycia), lub umowy najmu.';
            }
            else
            {
                echo 'W przypadku zawarcia przez ZAMAWIAJĄCEGO lub Jego bliskich lub osób pozostających w zależności służbowej umowy najmu nieruchomości podanej na Liście Wskazań Adresowych z pominięciem POŚREDNIKA, ZAMAWIAJĄCY zapłaci POŚREDNIKOWI karę umowną w wysokości dwukrotnego czynszu netto + vat (22%) - wg ceny podanej na Liście Wskazań Adresowych. Obowiązek zapłacenia tej kary wraz z odsetkami ustawowymi za każdy dzień zwłoki, powstaje z dniem zawarcia umowy najmu.';
            }
            echo '<li>';
            ...
            if ($transakcja == tags::$kupno)
            {
                echo 'Rozwiązanie niniejszej umowy przez ZAMAWIAJĄCEGO dokonane w celu zawarcia umowy kupna (nabycia) - także przez Jego bliskich lub osoby pozostające w zależności służbowej - wskazanej nieruchomości z pominięciem POŚREDNIKA - rodzi obowiązek zapłaty kary umownej określonej w punkcie 4.';
            }
            else
            {
                echo 'Rozwiązanie niniejszej umowy przez ZAMAWIAJĄCEGO dokonane w celu zawarcia umowy najmu - także przez Jego bliskich lub osoby pozostające w zależności służbowej - wskazanej nieruchomości z pominięciem POŚREDNIKA - rodzi obowiązek zapłaty kary umownej określonej w punkcie 4.';
            }
            echo '<li>';
            if ($transakcja == tags::$kupno)
            {
                echo 'POŚREDNIK nie ponosi odpowiedzialności za działania kupującego, sprzedającego, najmującego lub wynajmującego podjęte po zawarciu umowy kupna (nabycia), najmu a naruszające postanowienia tej umowy lub prawa i obowiązki stron.';
            }
            else
            {
                echo 'POŚREDNIK nie ponosi odpowiedzialności za działania kupującego, sprzedającego, najmującego lub wynajmującego podjęte po zawarciu umowy najmu a naruszające postanowienia tej umowy lub prawa i obowiązki stron.';
            }
            echo '<li>';
        ...
        public static function OfertaSzczegolowa($oferta, $jezyk_id, $agent_dane, $sekcje, $pomieszczenia, $pokaz_zdj = true)
        {
            ...
            UtilsUI::KonwersjaEncoding($tlumaczenia->Tlumacz($jezyk_id, $oferta[StronaOfertaDAL::$status]), $jezyk_id).'</span></td><td class="strtd" colspan="20"><b>'.
            ...
            $tekst .= '<tr><td class="strtd" colspan="2"><b>'.UtilsUI::KonwersjaEncoding($tlumaczenia->Tlumacz($jezyk_id, tags::$lokalizacja), $jezyk_id).': </b>'.UtilsUI::KonwersjaEncoding($oferta[StronaOfertaDAL::$lokalizacja], $jezyk_id).
