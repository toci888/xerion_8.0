        ...
        if (isset($_POST['oferta_id']))
        {
            ...
            if (isset($_POST['zatwierdz']) && isset($_SESSION[$param_nier.$_POST['oferta_id']]))
            {
               ...
               if (isset($_POST['prowizja_proc']))
                ...
                if (($_POST['status_id'] == $status_aktualny || $_POST['status_id'] == $status_umowiony) && (isset($_POST['pokaz']) || isset($_POST['portal'])))
                {
                    ...
                    {
                        $rezultat = $otodom->InsertOffer($_POST[NieruchomoscDAL::$id_oferta], $powodzenie, false, true);
                    }
                    else
                    {
                    ...
                    if (isset($_POST['portal']))
                    {
                        $rezultat = $domiporta->PublikujDomiporta($_POST[NieruchomoscDAL::$id_oferta], false, true);
                    }
                    else
                    {
            ...
            if (isset($_POST['edycja']) && isset($_POST['oferta_id']))
            {
                ...
                if($iloscWierszy == 1)
                {
                    ...
                    KontrolkiHtml::DodajSubmit('zatwierdz', 'id_zatwierdz', $tlumaczenia->Tlumacz ($_SESSION[$jezyk], tags::$zatwierdz).'.', 'onclick="return WalidacjaFormularz(Array(cena, prowizja, ulica), '.$tabPodpowiedzi.', \''.$tlumaczenia->Tlumacz($_SESSION[$jezyk], tags::$brak).' : \');"');
