...
<?php
    ...
    else
    {
        ...
        if (isset($_POST['drukuj']) && isset($_POST[NieruchomoscDAL::$id_oferta.'1']))
        {
            $wysokosc = 340;
            $dal = new StronaPodsInfoDAL();
            $rodz_trans_nier = $dal->RodzajOferta($_POST[NieruchomoscDAL::$id_trans_rodzaj.'1'], $_POST[NieruchomoscDAL::$id_nier_rodzaj.'1']);
            $rodz_trans_nier2 = $dal->RodzajOferta($_POST[NieruchomoscDAL::$id_trans_rodzaj.'2'], $_POST[NieruchomoscDAL::$id_nier_rodzaj.'2']);
            ...
            $styl_bkg_clr = 'style="background-color: '.$kolory_url[$_POST[NieruchomoscDAL::$id_nier_rodzaj.'1']].';"';
            $styl_bkg_clr2 = 'style="background-color: '.$kolory_url[$_POST[NieruchomoscDAL::$id_nier_rodzaj.'2']].';"';
            
            $tabParametr[NieruchomoscDAL::$id_oferta] = $_POST[NieruchomoscDAL::$id_oferta.'1'];
            
            $dane = $dal->OfertaSkroconaPojGablota($tabParametr, $ilosc_wierszy);
            $dane = $dane[0];
            
            $tabParametr[NieruchomoscDAL::$id_oferta] = $_POST[NieruchomoscDAL::$id_oferta.'2'];
            
            $dane2 = $dal->OfertaSkroconaPojGablota($tabParametr, $ilosc_wierszy);
            $dane2 = $dane2[0];
            ...
            echo '<table width="'.$szer.'"><tr>';
                ...
                {
                    echo '<td colspan="2"><img width="480" height="'.$wysokosc.'" src="../../media/'.$_POST[NieruchomoscDAL::$id_nieruchomosc.'1'].'/zdjecia/'.$_POST['zdjecia'].'" /></td>';
                }
                else
                {
                ...
                /////////------------------
                echo '<td width="490" border="1" rules="all" class="tableBorder">';
                echo '<table border="1" rules="all" class="tableBorder"><tr><td colspan="2" align="center" '.$styl_bkg_clr2.' class="naglowek_sama_gora"><b>'.$tlumaczenia->Tlumacz($_SESSION[$jezyk], $rodz_trans_nier2[0][NieruchomoscDAL::$transakcja_rodzaj]).' - '.
                $tlumaczenia->Tlumacz($_SESSION[$jezyk], $rodz_trans_nier2[0][NieruchomoscDAL::$nieruchomosc_rodzaj]).' - '.
                $tlumaczenia->Tlumacz($_SESSION[$jezyk], $dane2[NieruchomoscDAL::$rodzaj_budynek]);
                echo '</b></td></tr><tr><td colspan="2" align="center" class="czerwone_duze"><b>';
                if ($dane2[StronaPodsInfoDAL::$wylacznosc])
                {
                    echo $tlumaczenia->Tlumacz($_SESSION[$jezyk], tags::$oferta_na_wylacznosc);
                }
                else
                ...
                echo '</b></td></tr><tr>';
                if (isset($_POST['zdjecia2']))
                {
                    echo '<td colspan="2"><img width="480" height="'.$wysokosc.'" src="../../media/'.$_POST[NieruchomoscDAL::$id_nieruchomosc.'2'].'/zdjecia/'.$_POST['zdjecia2'].'" /></td>';
                }
                else
                {
                    echo '<td colspan="2" align="center" width="580" height="'.$wysokosc.'" class="naglowek_gora">'.$tlumaczenia->Tlumacz($_SESSION[$jezyk], $rodz_trans_nier2[0][NieruchomoscDAL::$nieruchomosc_rodzaj]).' <br /> '.
                    $tlumaczenia->Tlumacz($_SESSION[$jezyk], $dane[NieruchomoscDAL::$rodzaj_budynek]).'</td>';
                }
                //lokalizacja + cena, potem params + wypos
                
                echo '</tr><tr><td><b>'.$tlumaczenia->Tlumacz($_SESSION[$jezyk], tags::$lokalizacja).': </b>'.$dane2[StronaOfertaDAL::$lokalizacja].'</td><td><b>';
                $cena = number_format($dane2[NieruchomoscDAL::$cena], 0, ',', '.');
                echo $tlumaczenia->Tlumacz($_SESSION[$jezyk], tags::$cena).': </b>'.$cena.' PLN</td></tr>';
                $licz = 0;
                if (isset($_POST['parametry2']))
                foreach ($_POST['parametry2'] as $parametr)
                {
                    if ($licz % 2 == 0)
                    {
                        echo '<tr>';
                ...
                if (isset($_POST['wyposazenia2']))
                foreach ($_POST['wyposazenia2'] as $wyposazenie)
                {
                    if ($licz % 2 == 0)
                    {
                        echo '<tr>';
                ...
                echo '<b>'.$tlumaczenia->Tlumacz($_SESSION[$jezyk], tags::$numer_oferty).': '.$dane2[NieruchomoscDAL::$id_oferta].'.</b>';
                $komorka = number_format($dane2[OsobaDAL::$komorka], 0, '', '-');
                $telefon = number_format($dane2[OsobaDAL::$telefon], 0, '', '-');
                echo '<table width="484" border="1" rules="all" class="tableBorder"><tr><td align="left"><b><span class="naglowek_gora">'.$tlumaczenia->Tlumacz($_SESSION[$jezyk], tags::$prowadzacy_oferte).
                ':</span><br /></b><span style="color: black;">'.$dane2[NieruchomoscDAL::$agent].'.</span>';
                echo '<br /><span class="naglowek_gora"><b>'.$tlumaczenia->Tlumacz($_SESSION[$jezyk], tags::$telefon).': 0'.$komorka.', 0'.$telefon.'</b></span></td></tr>';
                echo '<tr><td align="left"><b><span class="naglowek_gora">'.$tlumaczenia->Tlumacz($_SESSION[$jezyk], tags::$wiecej_informacji_na_stronie_internetowej).': </span></b><br />';
                echo '<b><center><span class="napis_www">www.azg.pl</span></center></b></td></tr>';
                echo '</table>';
