...
{
    var wzor = /^[0-9,A-Z,a-z,.,\,,_,-]{1,20}(\@){1}[0-9,A-Z,a-z,.,-]{1,25}(.){1}[A-Z,a-z]{1,5}$/;
    if (!wzor.test(cntl.value))
    {
		if(cntl.value != "")
	    {
...
{
	setTimeout(AutoUzupelnianieSelect(parametr, evt, hidPamTym, hidPamTymP), 10);
}
...
function AutoScrollDown (url_str)
{
	if (!(navigator.userAgent.indexOf('MSIE') > -1))
	{
		...
		{
			document.body.scrollTop = position_top;
		}
	}
}
