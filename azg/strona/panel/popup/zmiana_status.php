...
//<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    ...
    else
    {
        ...
        echo '</td><td>';
        KontrolkiHtml::DodajTextboxData('data_do', 'id_data_do', $data_do, 10, 10, $tlumaczenia->Tlumacz($_SESSION[$jezyk], tags::$podana_inf_nie_jest_data), '', '', 'zm_status', '../');
        echo '</td><td>';
        KontrolkiHtml::DodajSubmit('pokaz', 'id_pokaz', $tlumaczenia->Tlumacz($_SESSION[$jezyk], tags::$pokaz), '');
        echo '</td></tr></table></form>';
        if (isset($_POST['pokaz']))
        {
            $wynik = $dal->PokazOdwiedziny(array(NieruchomoscDAL::$id_oferta => $oferta_id, NieruchomoscDAL::$data => $_POST['data'], NieruchomoscDAL::$data_do => $_POST['data_do']), $ilosc_wierszy);
            if ($ilosc_wierszy > 0)
            {
