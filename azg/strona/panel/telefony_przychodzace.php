...
<?php
    ...
    else
    {
        ...
        if (isset($_POST['uruchom_nasluch']))
        {
            $res = exec ('php /var/www/html/form/panel/automat_centralka.php > /dev/null &');
            //echo $res;
            //echo '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!';
        }
        ...
        {
            echo '<hr /><form method="POST" target="center" action="'.$_SERVER['PHP_SELF'].'">';
            KontrolkiHtml::DodajSubmit('uruchom_nasluch', 'id_uruchom_nasluch', $tlumaczenia->Tlumacz($_SESSION[$jezyk], $tlumaczenia->Tlumacz($_SESSION[$jezyk], tags::$uruchom_nasluch)).'.', '');
            echo '</form>';
        }
    }
    require('../stopka.php');
