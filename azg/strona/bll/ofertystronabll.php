<?php
    ...
    class StronaElemBLL
    {
        protected $plik_z_zapisem = 'zdjecia_o_firmie';
        protected $sciezka_zdj_o_firmie = '';
        protected $zdjecia_o_firmie;//tablica danych
        
        public function StronaElemBLL()
        {
            $this->sciezka_zdj_o_firmie = str_replace($_SERVER['PHP_SELF'], '', $_SERVER['SCRIPT_FILENAME']).'/tmp/zdjecia/';
            $this->zdjecia_o_firmie = cacheStrona::CzytajOfertaSpecjalna($this->plik_z_zapisem);
        }
        
        public function PodajIloscZdjec ()
        {
            return count($this->zdjecia_o_firmie);
        }
        
        public function DodajPlikOFirmie ($sciezka_upload, $rozszerzenie, $opis, $pozycja)
        {
            //opis w tagu zapewne, tag do bazy :P
            $ilosc = $this->PodajIloscZdjec();
            $pozycja = $pozycja - 1;
            if (isset($this->zdjecia_o_firmie[$pozycja]))
            {
                $temp = $this->zdjecia_o_firmie[$pozycja];
                $rozsz_zm_pliku = explode('.', $temp['zdjecie']);
                $this->zdjecia_o_firmie[$pozycja] = array('zdjecie' => 'o_firmie_'.$pozycja.'.'.$rozszerzenie, 'opis' => $opis);
                $this->zdjecia_o_firmie[] = $temp;
                rename($this->sciezka_zdj_o_firmie.'o_firmie_'.$pozycja.'.'.$rozsz_zm_pliku[1], $this->sciezka_zdj_o_firmie.'o_firmie_'.$ilosc.'.'.$rozsz_zm_pliku[1]);
            }
            else
            {
                $this->zdjecia_o_firmie[$pozycja] = array('zdjecie' => 'o_firmie_'.$pozycja.'.'.$rozszerzenie, 'opis' => $opis);
            }
            move_uploaded_file($sciezka_upload, $this->sciezka_zdj_o_firmie.'o_firmie_'.$pozycja.'.'.$rozszerzenie);
            //rename($sciezka_upload, $this->sciezka_zdj_o_firmie.'o_firmie_'.$pozycja.'.'.$rozszerzenie);
            cacheStrona::ZapiszOfertaSpecjalna($this->zdjecia_o_firmie, $this->plik_z_zapisem);
        }
        public function PodajPlikOFirmie()
        {
            return $this->zdjecia_o_firmie;
        }
    }
    ...
    {  
        ...
        public function PodajOferty($id_trans_rodzaj, $id_nier_rodzaj, &$ilosc_wierszy)
        {
            if ($this->filtrowanie == null) //troszke nadrobione i przekombinowane ale dziala
            {
                ...
                if ($this->ofertaPods == null)
                {
                    ...
                    $this->PobierzOferty($id_trans_rodzaj, $id_nier_rodzaj, $ilosc_wierszy);
                }
                else
        ...
        public function LiczbaOfertDzielnica ()
        {
        ...
        protected function PobierzOferty($id_trans_rodzaj, $id_nier_rodzaj, &$ilosc_ofert)
        {
            ...
            if ($this->filtrowanie != null)
            {
                ...
                $wynik = $this->dal->FiltracjaOfertaSkrocona($tabParametr, $ilosc_ofert);
            }
            else
            {
                $wynik = $this->dal->OfertaSkrocona($tabParametr, $ilosc_ofert);
            }
    ...
    class IloscOfertDzielnice
    {
        protected $dane;  //tablica
        protected $dal;
        protected $czas_powolania;
        protected $krok_cache = 120;
        
        public function IloscOfertDzielnice()
        {
            $this->dal = new StronaPodsInfoDAL();
            $this->czas_powolania = Utils::IloscSekund(date('Y'), date('m'), date('d'), date('G'), date('i'), date('s')); //czas, kiedy nastapil request osoby
        }
        
        public function PodajIloscOfert ($id_dzielnica, $id_trans_rodzaj, $id_nier_rodzaj, $tabParametr)
        {
            //odczyt z cache
            $this->dane = cacheOfertyDzielnica::CzytajDzielnica($id_dzielnica, $id_trans_rodzaj, $id_nier_rodzaj);
            if ($this->dane == null)
            {
                $this->dane['czas'] = $this->czas_powolania;
                $wynik = $this->dal->IloscOfertRegion($tabParametr);
                $this->dane['ilosc'] = $wynik;
                cacheOfertyDzielnica::ZapiszDzielnica($id_dzielnica, $id_trans_rodzaj, $id_nier_rodzaj, $this->dane);
                
                return $wynik;
            }
            else if ($this->dane['czas'] < ($this->czas_powolania - $this->krok_cache))
            {
                $this->dane['czas'] = $this->czas_powolania;
                $wynik = $this->dal->IloscOfertRegion($tabParametr);
                $this->dane['ilosc'] = $wynik;
